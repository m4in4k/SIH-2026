    ...
    for observation in db.observations.find({**query,'txid':{'$in':list(chosen)}}).limit(100):
        for field in ('src_ip','dst_ip'):
            ...
            graph.add_node(node_id,kind='ip',label=ip+(' · '+' · '.join(map(str,qualifiers)) if qualifiers else ''),focus=False,
                           country=country,country_code=geo.get('country_code'),asn=asn,as_org=geo.get('as_org'),
                           geo_source=geo.get('source'),related_txids=related)
            graph.nodes[observation['txid']]['related_ips']=sorted(
                set(graph.nodes[observation['txid']].get('related_ips',[])) | {ip}
            )
            if side=='src':
...
def case_clusters(case_id:str,cluster_type:str=Query('all',pattern='^(all|wallet|ip)$'),user=Depends(current_user)):
